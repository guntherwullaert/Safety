...
\begin{document}
	...
	\begin{alignat*}{4}
		\Eval(c) &= \{c\} & \\
		\Eval(a + b) &= \{ x + y &&\mid x \in \Eval(a), y \in \Eval(b) \} \\
		\Eval(a - b) &= \{ x - y &&\mid x \in \Eval(a), y \in \Eval(b) \} \\
		\Eval(a * b) &= \{ x * y &&\mid x \in \Eval(a), y \in \Eval(b) \} \\
		\Eval(a / b) &= \{ x / y &&\mid x \in \Eval(a), y \in \Eval(b), y \neq 0 \}
	\end{alignat*}
	We say that a term $c$ is \emph{nonzero} if it is numeric and $0 \notin \Eval(c)$.
  ...
  %
	...
	\begin{itemize}
		...
		\item
			for a term of form $f(\pool{t})$, we let
			$\Provide(f(\pool{t})) = \Provide(\pool{t})$,
		\item
			for a term of form $t_1 \star t_2$, we let
				\begin{align*}
				\Provide(t_1 \star t_2) = \begin{cases}
					\Provide(t_2) & \text{\shortstack[l]{$t_1$ is numeric and ${\star} \in \set{{+}, {-}}$}, or}\\
					              & \text{\shortstack[l]{$t_1$ is nonzero and ${\star} = {\times}$,}}\\
					\Provide(t_1) & \text{\shortstack[l]{$t_2$ is numeric and ${\star} \in \set{{+}, {-}}$}, or}\\
					              & \text{\shortstack[l]{$t_2$ is nonzero and ${\star} = {\times}$,}}\\
					\emptyset & \text{otherwise}
				\end{cases}
				\end{align*}
	\end{itemize}
	We define function $\Depend$ for a term $t$ as $\Depend(t) = \Vars(t) \setminus \Provide(t)$.
